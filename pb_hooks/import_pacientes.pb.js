...
routerAdd('POST', '/api/custom/import-pacientes', (c) => {
  try {
    ...
    // ─── 2. Parse JSON body ───
    // c.parseBody() não funciona com Content-Type: application/json
    // Usa JSON.parse(c.bodyString) + fallback
    let body;
    try {
      body = JSON.parse(c.bodyString || '{}');
    } catch (_) {
      try {
        body = c.parseBody();
      } catch (_) {
    ...
    return c.json(500, {
      code: 500,
      message: e.message || 'Erro interno inesperado',
    });
  }
});
