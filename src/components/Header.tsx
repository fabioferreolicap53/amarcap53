  ...
  return (
    <header className="bg-gradient-to-r from-[#001b3d] to-[#002b5c] backdrop-blur-md shadow-[0px_8px_32px_rgba(0,0,0,0.3)] flex items-center w-full px-4 md:px-6 h-[80px] sticky top-0 z-40 border-b border-white/10">
      ...
      <div className="hidden lg:flex items-center h-full flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 lg:gap-5 xl:gap-8 flex-gap-fallback min-w-0 flex-1 h-full">
          {/* Navegação */}
          <nav className="flex items-center justify-center gap-1 lg:gap-1.5 xl:gap-2 flex-gap-fallback overflow-x-auto no-scrollbar pb-1 min-w-0">
            {navItems.map((item) => {
          ...
          {user && (
            <div className="hidden md:flex items-center gap-3 lg:gap-4 xl:gap-5 flex-gap-fallback min-w-0 border-l border-white/10 pl-3 lg:pl-4 xl:pl-5 h-full shrink-0">
              {/* Unidade */}
              {user.unidade_saude && (
                <div className="flex items-center gap-1.5 lg:gap-2 xl:gap-2.5 flex-gap-fallback group/info min-w-0 shrink-0 max-w-[180px] lg:max-w-[220px] xl:max-w-none">
              ...
              {/* Equipe */}
              {(user.role === 'equipe' || user.role === 'microarea') && user.equipe && (
                <div className="flex items-center gap-1.5 lg:gap-2 xl:gap-2.5 flex-gap-fallback group/info min-w-0 shrink-0 max-w-[120px] lg:max-w-[140px] xl:max-w-none">
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-gap-fallback h-full shrink-0 ml-2">
            <div className="flex items-center gap-2 md:gap-3 flex-gap-fallback pl-2 md:pl-4 border-l border-white/10 h-full">
